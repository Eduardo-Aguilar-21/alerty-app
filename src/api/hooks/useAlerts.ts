...
export const useAlert = (companyId?: number, id?: number) => {
  return useQuery<AlertDetail, Error>({
    ...
    queryFn: () =>
      alertService.getAlertById(companyId as number, id as number),
  });
};
...
export const useUpdateAlert = () => {
  ...
  >({
    mutationFn: (args) =>
      alertService.updateAlert(args.companyId, args.id, args.data),
...
export const useDeleteAlert = () => {
  ...
  return useMutation<void, Error, { companyId: number; id: number }>({
    mutationFn: ({ companyId, id }) =>
      alertService.deleteAlert(companyId, id),
    onSuccess: (_data, vars) => {
      queryClient.invalidateQueries({
        queryKey: ["alert", vars.companyId, vars.id],
      });
      queryClient.invalidateQueries({ queryKey: ["alerts"] });
    },
  });
...
export const useAcknowledgeAlert = () => {
  ...
  return useMutation<AlertDetail, Error, { companyId: number; id: number }>({
    mutationFn: ({ companyId, id }) =>
      alertService.acknowledgeAlert(companyId, id),
