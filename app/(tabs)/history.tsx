...
export default function HistoryScreen() {
    const router = useRouter(); // 👈 AQUÍ creamos el router
    ...
    const pendingOnPage = filteredAlerts.filter(
        (a) => !a.acknowledged
    ).length;
    const criticalOnPage = filteredAlerts.filter(
    ...
    // 👇 AHORA SÍ NAVEGA AL DETALLE
    const handleViewDetails = (alert: AlertSummary) => {
        router.push({
            pathname: "/alert/[id]",       // 👈 coincide con app/alert/[id].tsx
